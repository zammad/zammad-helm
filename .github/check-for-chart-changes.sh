...
CHART_REPO="https://github.com/zammad/zammad-helm.git"
...
if [ -z "$(git diff --find-renames --name-only "$(git rev-parse --abbrev-ref HEAD)" remotes/chart-changes/master -- zammad)" ]; then
