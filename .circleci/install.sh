...
main() {
    ...
    docker exec "$config_container_id" ct install --config=${WORKDIR}/.circleci/ct.yaml --lint-conf=${WORKDIR}/.circleci/lintconf.yaml --chart-yaml-schema=${WORKDIR}/.circleci/chart_schema.yaml
